...
/**
 * PUBLIC_INTERFACE
 * Onboarding form to capture team basics: name, size, work mode.
 */
export default function Onboarding() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await actions.setTeam({ name, size: Number(size), mode });
    ...
    <Container>
      ...
      <Card>
        ...
        <div className="mt-4" style={{ display: 'flex', gap: 12 }}>
          ...
          <Button onClick={handleSave} disabled={!canContinue || saving}>{saving ? 'Saving…' : 'Continue to Quiz'}</Button>
        </div>
      </Card>
    </Container>
  );
